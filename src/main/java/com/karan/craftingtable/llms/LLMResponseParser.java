...
public class LLMResponseParser {
    ...
    public List<ChatEventEntity> parseChatEvents(String fullResponse, ChatMessageEntity parentMessage) {
        List<ChatEventEntity> events = new ArrayList<>();
        int orderCounter = 1;
        ...
        while (matcher.find()) {
            ...
            ChatEventEntity.ChatEventEntityBuilder builder = ChatEventEntity.builder()
                    .chatMessage(parentMessage)
                    .content(content) // This is your Markdown content
                    .sequenceOrder(orderCounter++);

            switch (tagName) {
                case "message" -> builder.chatEventType(ChatEventTypeEnum.MESSAGE);
                case "file" -> {
                    builder.chatEventType(ChatEventTypeEnum.FILE_EDIT);
                    builder.filePath(attrMap.get("path")); // Required for files
//                    builder.content(null);
                }
                case "tool" -> {
                    builder.chatEventType(ChatEventTypeEnum.TOOL_LOG);
